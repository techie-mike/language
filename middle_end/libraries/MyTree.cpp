...
void Tree::whatItIs(char *name, size_tree_t index)
{
   if (isOperator(name, index))
       return;
   if (isNumber(name, index))
       return;
   if (isVariable(name, index))
       return;
}
...
bool Tree::isVariable(char *name, size_tree_t index) {
    ...
    #define NAME(name_var) if (!strcmp(name , name_var)) {\
        return false;\
    } else
    ...
    NAME ("=")
    NAME (",")
//    #include "../../frontend/libraries/def_com_language.h"
#undef NAME
    if (*name == '$'){
        one_element[index].type_ = TYPE_FUNCTION;
        return true;
    } else {
        one_element[index].type_ = TYPE_VARIABLE;
        return true;
...
void Tree::allSimplifications(char *text) {
    ...
    while (true){
        int num_action = 0;
        num_action += optimisationOfConstants();
        num_action += optimisationUnusedMembers();
        if (!num_action)
            break;
    }
}

int Tree::optimisationOfConstants() {
...
int Tree::optimisationUnusedMembers(bool main_func) {
    int num_optimisation = 0;
//    dump();
    while (true) {
        size_tree_t branch = searchUnusedNode(this, root_);
        if (!branch) break;
        optimisationOfUnusedNode (branch, main_func);
//        recordExpression(text);
//        writeTexInText(text, index);
//        strcat(text, "\n");
//        dump();
...
void Tree::optimisationOfUnusedNode(size_tree_t index, bool main_func) {
    switch ((int) (one_element[index].value_ + 1e-12)) {
...
bool Tree::readTreeFromFile(int num_arg, char *poin_arg[]) {
    char* name_file = nullptr;
    if (num_arg > 1) {
        name_file = poin_arg[1];
    } else  {
...
size_tree_t Tree::searchDeriv(size_tree_t index) {
    if (index == 0)
        return 0;
    if (!strcmp (one_element[index].name_, "deriv"))
        return index;
    size_tree_t return_index = searchDeriv (one_element[index].left_);
    if (return_index)
        return return_index;
    return_index = searchDeriv (one_element[index].right_);
    if (return_index)
        return return_index;
    return 0;
}
...
void Tree::diffFunc() {
    while (true){
        size_tree_t index = searchDeriv (root_);
        if (!index)
            break;
        size_tree_t diff_index = diff (this, one_element[index].right_, one_element[one_element[index].left_].name_);

        size_tree_t last_parent = one_element[index].parent_;
        bool left = false;
        if (one_element[last_parent].left_ == index)
            left = true;
        clearBranch (index);

        one_element[diff_index].parent_ = last_parent;
        if (left)
            one_element[last_parent].left_ = diff_index;
        else
            one_element[last_parent].right_ = diff_index;
...
void Tree::secondLineView (size_tree_t index) {
    if (strcmp(one_element[index].name_, "$main") != 0)
     if (index != 0 && !functionsView (index) && !assignmentView (0, index))
         printf ("Error, unknown branch from main line!\n");
}

bool Tree::assignmentView (nameTable* table, size_tree_t index) {
...
//        assignmentWriteInTextCode ();
        ...
        mathOperatorsView (table, one_element[index].right_);

        writeAssignmentVariable (table, one_element[index].left_);
...
bool Tree::functionsView (size_tree_t index) {
    if (*one_element[index].name_ == '$') {
        ...
        searchAllVariablesInFuncView (&table, one_element[index].left_);
        table.num_arg_ = table.size_ + 1;
        table.createNameInTable (" ");
        searchAllVariablesInFuncView (&table, one_element[index].right_);

        if (!strcmp (one_element[index].name_, "$main")) {
            writeNameInTextCode ("push ");
            writeNameInTextCode (table.num_arg_);
            writeNameInTextCode ("\n"
                                 "pop bp\n"
                                 "call :");
        ...
        writeStartFunction (index);

//        label.createNameInTable (one_element[index].name_ + 1);

        writeNameInTextCode (":");
        writeNameInTextCode (label.searchNameInTable (one_element[index].name_ + 1));
        writeNameInTextCode ("\n");

        lineOfFunctionsView (&table, one_element[index].right_);

        writeEndFunction (index);
        if (!strcmp (one_element[index].name_, "$main"))
            writeNameInTextCode ("end\n");
        return true;
    }
...
void Tree::returnOldValueBp(nameTable* table) {
    writeNameInTextCode ("push bp; return last bp\n"
                         "push -1\n"
                         "add\n"
                         "pop dx\n"
                         "push [dx]\n"
...
void Tree::writeConvertCode (const char* name_file) {
    ...
    write_text_ = text;
    num_write_text_ = 0;
    createAllFunctionLabel (root_);
    free_label_ = label.size_;
...
void Tree::lineOfFunctionsView(nameTable* table, size_tree_t index) {
    if (index) {
        if (strcmp(one_element[index].name_, "op") != 0)
            printf ("Error in operators of functions!\n");

        operatorsView (table, one_element[index].left_);

        lineOfFunctionsView (table, one_element[index].right_);
    }
}

void Tree::operatorsView(nameTable* table, size_tree_t index) {
    if (index == 0)
...
bool Tree::callFunctionsView (nameTable* table, size_tree_t index) {
    if ((one_element[index].type_ == TYPE_OPERATOR*//*

*/
/* || !strcmp(one_element[index].name_, "=")*//*
*/
/*
) && (*one_element[one_element[index].right_].name_ == '$'*//*

*/
/* || *one_element[one_element[index].left_].name_ == '$'*//*
*/
/*
)) {
//        writeNameInTextCode (one_element[one_element[index].left_].name_);
...
//        assignmentWriteInTextCode ();
        ...
        callFunctionsView (table, one_element[index].right_);
...
bool Tree::operatorIfView(nameTable* table, size_tree_t index) {
    if (!strcmp (one_element[index].name_, "if")) {
        int using_label = free_label_;
        free_label_++;

//        writeNameInTextCode (name_if);
//        writeNameInTextCode (" ");
        compareView (table, one_element[index].left_, using_label);
//        writeNameInTextCode (" ");


//        num_tabs_++;
        allResultIfView (table, one_element[index].right_, using_label);
//        num_tabs_--;
...
void Tree::compareView(nameTable* table, size_tree_t index, int using_label) {
//    writeNameInTextCode ("(");
    writeValueNumberAndVariable (table, one_element[index].left_);
    writeValueNumberAndVariable (table, one_element[index].right_);
//    writeNameInTextCode (one_element[one_element[index].left_].name_);
//    writeNameInTextCode (one_element[one_element[index].right_].name_);
//    writeNameInTextCode (" ");
    if (!strcmp (one_element[index].name_, "=="))
        writeNameInTextCode ("je ");
    if (!strcmp (one_element[index].name_, "!="))
        writeNameInTextCode ("jne ");
    if (!strcmp (one_element[index].name_, "<"))
        writeNameInTextCode ("jb ");
    if (!strcmp (one_element[index].name_, ">"))
        writeNameInTextCode ("ja ");
    if (!strcmp (one_element[index].name_, ">="))
        writeNameInTextCode ("jae ");
    if (!strcmp (one_element[index].name_, "<="))
        writeNameInTextCode ("jbe ");

    writeLabel (using_label);
...
bool Tree::oneMathOperatorView (nameTable* table, size_tree_t index, const char* name, size_tree_t type,
...
/*     if (last_priority > now_priority) {
            writeNameInTextCode ("(");
            mathOperatorsView (table, one_element[index].left_, now_priority);
            writeNameInTextCode (" ");
            writeNameInTextCode (real_name);
            writeNameInTextCode (" ");
            mathOperatorsView (table, one_element[index].right_, now_priority);
            writeNameInTextCode (")");
        } else {*//*
*/
/*
...
void Tree::mathOperatorsView (nameTable* table, size_tree_t index, int last_priority) {
    if (!index)
        return;
    if (oneMathOperatorView (table, index, "+",   TYPE_OPERATOR, OPERATOR_ADD, "add\n",       last_priority))
        return;
    if (oneMathOperatorView (table, index, "-",   TYPE_OPERATOR, OPERATOR_SUB, "sub\n",    last_priority))
        return;
    if (oneMathOperatorView (table, index, "*",   TYPE_OPERATOR, OPERATOR_MUL, "mul\n", last_priority))
        return;
    if (oneMathOperatorView (table, index, "/",   TYPE_OPERATOR, OPERATOR_DIV, "div\n",       last_priority))
        return;
    if (oneMathOperatorView (table, index, "^",   TYPE_OPERATOR, OPERATOR_POW, "pow\n",          last_priority))
        return;
...
*/
/*writeNameInTextCode ("push [");
        writeNameInTextCode (table->searchNameInTable (one_element[index].name_)
        - table->num_arg_);*//*
*/
/*
...
*/
/*void Tree::operatorDerivView(nameTable* table, size_tree_t index) {
    writeNameInTextCode ("d(");
    mathOperatorsView (table, one_element[index].right_, 3);
    writeNameInTextCode (")/d ");
    writeNameInTextCode (one_element[one_element[index].left_].name_);
...
void Tree::allResultIfView(nameTable* table, size_tree_t index, int using_label) {
    if (!one_element[index].left_)
        printf ("Error, \"if\" haven't main result");

    resultIfView (table, one_element[index].left_);
...
//        num_tabs_++;
        ...
        resultIfView (table, one_element[index].right_);

    }
}

void Tree::resultIfView (nameTable* table, size_tree_t index) {
//    if (!index) return;
//    if (one_element[index].right_ == 0){
//        writeNameInTextCode ("\n");
        lineOfFunctionsView (table, index);
   *//*
...
bool Tree::operatorPutView (nameTable* table, size_tree_t index) {
    if (!strcmp (one_element[index].name_, "put")) {

//        writeNameInTextCode (" ");
        mathOperatorsView (table, one_element[index].left_);
//        writeNameInTextCode (one_element[one_element[index].left_].name_);
        writeNameInTextCode ("out\n\n");
...
bool Tree::operatorGetView (nameTable* table, size_tree_t index) {
    if (!strcmp (one_element[index].name_, "get")) {
        writeNameInTextCode ("in\n");
        writeAssignmentVariable (table, one_element[index].left_);
//        writeNameInTextCode (" ");
//        writeNameInTextCode (one_element[one_element[index].left_].name_);
//        writeNameInTextCode ("\n");
...
bool Tree::operatorWhileView (nameTable* table, size_tree_t index) {
    if (!strcmp (one_element[index].name_, "while")) {
        int cycle_label = free_label_;
        free_label_++;
        int comp_label = free_label_;
        free_label_++;

//        writeNameInTextCode (name_while);
//        writeNameInTextCode (" ");
        writeLabel (cycle_label);
        writeNameInTextCode ("; start while\n");
        compareView (table, one_element[index].left_, comp_label);
...
//        num_tabs_++;
        ...
        writeNameInTextCode ("jmp ");
        writeLabel (cycle_label);
        writeLabel (comp_label);
        writeNameInTextCode ("; end while\n\n");
//        num_tabs_--;
...
bool Tree::operatorReturnView(nameTable* table, size_tree_t index) {
    if (!strcmp (one_element[index].name_, "ret")) {

        mathOperatorsView (table, one_element[index].left_);
        if (one_element[index].left_ != 0)
            writeNameInTextCode ("pop cx\n");

        writeNameInTextCode ("ret\n");
...
void Tree::searchMainFunctionView(size_tree_t index) {
    if (!strcmp(one_element[index].name_, ";")) {
        if (!strcmp(one_element[one_element[index].left_].name_, "$main")) {
            functionsView (one_element[index].left_);
            return;
        }
        if (one_element[index].right_)
            searchMainFunctionView (one_element[index].right_);
    } else
        printf ("Error first line program");
}

void Tree::searchAllVariablesInFuncView (nameTable* table, size_tree_t index) {
    if (index) {
        if (one_element[index].type_ == TYPE_VARIABLE
        && !checkNameVariable (table, one_element[index].name_)){
            table->createNameInTable (one_element[index].name_);
        }
        searchAllVariablesInFuncView (table, one_element[index].left_);
        searchAllVariablesInFuncView (table, one_element[index].right_);
    }
...
bool Tree::checkNameVariable (nameTable* table, char* name) {
    if (table->searchNameInTable (name) != -1)
        return true;
    else return false;
}

int Tree::writeArgumentFunction (nameTable* table, size_tree_t index) {
    int num_offset = 0;
    if (index) {
        size_tree_t end_index = index;
        while (one_element[index].left_ != 0)
            index = one_element[index].left_;

        while (index != end_index) {
            if (one_element[one_element[index].right_].type_ == TYPE_VARIABLE)
                copyArguments (table, index, num_offset);
            if (one_element[one_element[index].right_].type_ == TYPE_OPERATOR) {
                mathOperatorsView (table, one_element[index].right_);
                writeCopyArgument (table, index, num_offset);
            }

            index = one_element[index].parent_;
            num_offset++;
        }
        writeOldValueBp (table, num_offset);
    }
    return num_offset;
}

void Tree::createNewValueBp(nameTable* table, int num_offset) {
    writeNameInTextCode ("push bp\n"
                         "push ");
    writeNameInTextCode (table->size_+ num_offset - table->num_arg_ + 1);
//    char text_num[100] = {};
//    sprintf (text_num, "%d", table->size_);
//    writeNameInTextCode (text_num);
    writeNameInTextCode ("\n"
                         "add\n"
                         "pop bp; index in ram on new function\n\n");
}

void Tree::copyArguments(nameTable* table, size_tree_t index, int num_offset) {
    char command[200] = {
            "push bp\n"
            "push "
    };
    ntable_t index_variable_ram
            = table->searchNameInTable (one_element[one_element[index].right_].name_)
                    -  table->num_arg_;
    if (index_variable_ram == -1)
        printf ("Error, unknown variable in argument of function!\n");

    char text_num[100] = {};
    sprintf (text_num, "%d", index_variable_ram);
    strcat (command, text_num);
    strcat (command, "\n"
                     "add\n"
                     "pop dx\n"
                     "push [dx]; original ");
    strcat (command, one_element[one_element[index].right_].name_);
    strcat (command, "\n\n");

    strcat (command, "push bp\n"
                     "push ");

    ntable_t now_write_position = num_offset + table->size_ - table->num_arg_;
    memchr (text_num, '\0', strlen (text_num) + 1);
    sprintf (text_num, "%d", now_write_position);
    strcat (command, text_num);
    strcat (command, "\n"
                     "add\n"
                     "pop dx\n"
    ...
    writeNameInTextCode (command);
}

void Tree::writeOldValueBp(nameTable* table, int num_offset) {
    writeNameInTextCode (
            "push bp\n"
            "push bp\n"
            "push ");

    ntable_t num_busy = table->size_ + num_offset - table->num_arg_;
//    char text_num[100] = {};
//    sprintf (text_num, "%d", num_busy);
    writeNameInTextCode (num_busy);
    writeNameInTextCode ("\n"
                     "add\n"
                     "pop dx\n"
                     "pop [dx]; write old bp\n");
...
void Tree::writeNameInTextCode(int name_num) {
    char text_num[100] = {};
    sprintf (text_num, "%d", name_num);
    writeNameInTextCode (text_num);
}

void Tree::createAllFunctionLabel(size_tree_t index) {
    if (!strcmp(one_element[index].name_, ";")) {
        if (*one_element[one_element[index].left_].name_ == '$') {
            char* name = one_element[one_element[index].left_].name_ + 1;
            if (label.searchNameInTable (name) == -1)
                label.createNameInTable (name);
        }
        if (one_element[index].right_)
            createAllFunctionLabel (one_element[index].right_);
    } else
        printf ("Error first line program");
}

void Tree::writeAssignmentVariable(nameTable* table, size_tree_t index) {
    writeNameInTextCode ("push ");
//    mathOperatorsView (table, index);
    writeNameInTextCode (table->searchNameInTable (one_element[index].name_) - table->num_arg_);
    writeNameInTextCode ("\n"
                         "push bp\n");
//    writeNameInTextCode (table->num_arg_);
//    writeNameInTextCode ("sub\n");
    writeNameInTextCode ("add\n");
    writeNameInTextCode ("pop dx\n");
    writeNameInTextCode ("pop [dx]; assignment in variable \"");

    writeNameInTextCode (one_element[index].name_);
    writeNameInTextCode ("\"\n\n");
}

void Tree::writeValueVariable(nameTable* table, size_tree_t index) {
    if (one_element[index].type_ == TYPE_VARIABLE) {
        writeNameInTextCode ("push ");
        writeNameInTextCode (table->searchNameInTable (one_element[index].name_)
                             - table->num_arg_);
...
void Tree::writeValueNumber(nameTable* table, size_tree_t index) {
    if (one_element[index].type_ == TYPE_NUMBER) {
        writeNameInTextCode ("push ");
        writeNameInTextCode (one_element[index].name_);
        writeNameInTextCode ("\n");
...
void Tree::writeValueNumberAndVariable(nameTable* table, size_tree_t index) {
    writeValueVariable (table, index);
    writeValueNumber (table, index);
}

void Tree::writeLabel(int using_label) {
    writeNameInTextCode (":");
    writeNameInTextCode (using_label);
    writeNameInTextCode ("\n");
}

void Tree::writeStartFunction(size_tree_t index) {
    writeNameInTextCode ("; start function \"");
    writeNameInTextCode (one_element[index].name_ + 1);
    writeNameInTextCode ("\"\n");
}

void Tree::writeEndFunction(size_tree_t index) {
    writeNameInTextCode ("; end function \"");
    writeNameInTextCode (one_element[index].name_ + 1);
    writeNameInTextCode ("\"\n");
}

void Tree::writeCopyArgument(nameTable* table, size_tree_t index, int num_offset) {
    char command[200] = {};
